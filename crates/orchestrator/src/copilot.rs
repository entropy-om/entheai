//! `CoderExecutor` impl: run each fan-out coder sub-task via the GitHub Copilot
//! CLI (`copilot`) inside its isolated worktree, leaving UNCOMMITTED changes for
//! `run_fanout`'s commit/verify/integrate path — entheai orchestrates + integrates
//! while Copilot (OpenAI/Anthropic models behind GitHub) does the coding.
//!
//! Same shape and safeguards as the [`crate::agy`] path:
//!   * a DEPTH GUARD — each spawn increments `ENTHEAI_FANOUT_DEPTH`; past `MAX_DEPTH`
//!     the executor declines (`workers_available()` → false → local fallback), so a
//!     coder that itself invokes entheai can't recurse without bound;
//!   * a LAYER-AWARE PREAMBLE — every coder prompt carries its layer + role and an
//!     explicit "stay in your layer, don't start another fan-out".
//!
//! Non-interactive invocation (headless subprocess — no TTY to answer prompts):
//! `copilot -p <prompt> --allow-all-tools --no-color --add-dir <worktree>` with
//! `--model` when configured, run with `current_dir` = the worktree. Under a
//! Spawn-ceiling mode it also passes `--allow-all-paths --allow-all-urls`. Auth is
//! inherited from the environment (`COPILOT_GITHUB_TOKEN` / `GH_TOKEN` /
//! `GITHUB_TOKEN`). Any failure (copilot missing, error, no change) returns `None`
//! → local fallback.

use std::path::Path;
use std::sync::Arc;

/// Env var threading the current fan-out depth to nested entheai/copilot processes.
...
pub struct CopilotExecutor {
    /// Model to pass via `--model` (empty = the Copilot CLI's own default).
    model: String,
    depth: u32,
}
...
impl CopilotExecutor {
    ...
    /// (0 at the top level); each `execute` spawns copilot at `depth + 1`.
    /// `model` may be empty to defer to the Copilot CLI's configured default.
...
/// followed by the focused task. Pure — unit-tested. Mirrors [`crate::agy`]'s so
/// each executor file stays self-contained.
...
impl crate::CoderExecutor for CopilotExecutor {
    async fn workers_available(&self) -> bool {
        if self.depth >= MAX_DEPTH {
            log::warn!(
                "copilot executor: fan-out depth {} ≥ max {MAX_DEPTH} — coders run locally (recursion guard)",
                self.depth
            );
            return false;
        }
    ...
    async fn execute(
        ...
        let widest = mode.ceiling() == entheai_permission::Tier::Spawn;

        let mut cmd = tokio::process::Command::new("copilot");
        cmd.arg("-p")
            .arg(&prompt)
            .arg("--no-color")
            // A headless subprocess has no TTY to answer permission prompts, so it
            // MUST auto-approve tools or it can't edit/run anything. Paths stay
            // scoped to the worktree via `current_dir` + `--add-dir`.
            .arg("--allow-all-tools")
            .arg("--add-dir")
            .arg(worktree_path);
        if widest {
            // Yolo/Spawn ceiling: lift path + URL verification too.
            cmd.arg("--allow-all-paths").arg("--allow-all-urls");
        }
        ...
        if !out.status.success() {
            log::warn!(
                "copilot coder (layer {}, {role}) failed → local fallback",
                self.depth
            );
            return None;
        }

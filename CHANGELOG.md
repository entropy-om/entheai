...
- **Prompt-processing — native in-process mesh (default).** A `NativeMesh` re-ranks candidate raw spans **in-process with no subprocess** (`[memory.prompt_processing] mesh_backend = "native"`, now the default), dropping the Python sidecar from the default path while keeping it available (`mesh_backend = "sidecar"`). It scores each candidate with an optional `.ugm` reranker (`native_model`, a `FEATURE_DIM=512`-input dense model consuming `[query|text]` byte-histograms, run via `entheai-ultragraph`) or a deterministic lexical fallback. PP now completes end-to-end — recall → native re-rank → rehydrate raw → compress — with zero external tools.

...
- **`entheai-vision` — an image post-processing layer.** New crate analyzing a pasted image (raw bytes, MIME-sniffed) or a file path (MIME by extension, falling back to sniffing), preferring the `agy` (Antigravity CLI / Gemini) backend — `[fanout].agy_model` — and falling back to a configured vision-capable `adk_rust` model (e.g. a local Gemma/hf-mac endpoint) on any failure: missing binary, non-zero exit, timeout, or empty output. Exposed via `entheai --image <path>` (or `--image -` to read pasted/piped bytes from stdin), with the positional prompt as the instruction.

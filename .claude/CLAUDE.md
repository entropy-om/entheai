...
## Codebase Intelligence for entheai (Repowise)

Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-07-19 (commit 6bb7ae3)
...
- `crates/core/src/lib.rs` — 15 commits/90d (97.6th %ile)
- `crates/tui/src/lib.rs` — 12 commits/90d (95.1th %ile)
- `crates/providers/src/lib.rs` — 12 commits/90d (92.7th %ile)
- `bin/entheai/src/main.rs` — 16 commits/90d (90.2th %ile)
- `crates/companion/src/render.rs` — 4 commits/90d (87.8th %ile)

### Code health
Three co-equal signals: defect risk 8.26/10 avg, hotspot health 4.82/10 (stable), worst `crates/core/src/lib.rs` at 1.9/10 · maintainability 8.71/10 · performance risk 24 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `bin/entheai/src/main.rs` — change entropy — impact −1.4
- `crates/core/src/lib.rs` — change entropy — impact −1.1
- `bin/entheai/src/main.rs` — function hotspot (main) — impact −1.0
- `crates/core/src/lib.rs` — prior defect — impact −0.7
- `crates/tui/src/lib.rs` — large method (event_loop) — impact −0.4
